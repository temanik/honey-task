...
// - Однако, после перезапуска источник начнет с прошлой "подтвержденной"
//   позиции, задаваемой cookie.
// - Поэтому каждое значение cookie, которое вернул вызов Next, после
//   сохранения данных в приемнике, должно быть фиксировано вызовом Commit,
//   причем строго в той же последовательности, в которой их вернул Next.
...
// которая читает данные из источника, группирует их в буфер размером не более
// MaxItems и сохраняет в приемник, после чего фиксирует прогресс в источнике.
